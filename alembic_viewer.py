...
class GraphCanvas(tk.Canvas):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.on_node_double_click: Callable[[str], None] | None = None  # Callback para doble-click
        self.on_node_deselect: Callable[[], None] | None = None  # Callback para deselección
...
class AlembicViewerApp:
    ...
    def _setup_ui(self):
        ...
        if HAS_TKCALENDAR:
            ttk.Button(
                filter_frame, text="📅", width=3, command=lambda: self._show_calendar_popup(self.date_from_var)
        ...
        if HAS_TKCALENDAR:
            ttk.Button(
                filter_frame, text="📅", width=3, command=lambda: self._show_calendar_popup(self.date_to_var)
            ).pack(side=tk.LEFT, padx=(2, 0))

        ttk.Button(filter_frame, text="Aplicar Filtro", command=self._apply_date_filter).pack(
...
Ejemplos:
    ...
    icon_path = Path(__file__).parent / "alembic_viewer.ico"
    if icon_path.exists():
        try:
            root.iconbitmap(str(icon_path))
        except tk.TclError:
            pass  # Algunos sistemas no soportan .ico directamente
